...
`/games/prompdojo/play` 配下で壊れる典型例は、次の 3 つです。
...
- `assetPrefix` は `_next` 配下の asset を mount path 経由で配信するために使う
...
- 生成 HTML の `link` / `script` が `/games/prompdojo/play/_next/...` を向いていることを確認する
...
- `/_next/...` になっていたら mount path 配信に失敗している
- `next.config.ts` の `PUBLIC_MOUNT_PREFIX` と `assetPrefix` を見直す
...
- `/_next` 配下の CSS / JS が mount path 経由で配信される前提を壊していない

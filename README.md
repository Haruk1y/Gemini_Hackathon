...
- `IMAGE_PROVIDER_DEFAULT` controls the default lobby setting for new rooms. Players can still switch the room between `gemini` and `flux` while the room is in the lobby.
...
If you want to use Flux locally, authenticate Application Default Credentials before starting the app:
...
- Room settings include an `imageModel` field, so target image generation and player answer generation use the same provider per room.
